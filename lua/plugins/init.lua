        ...
        config = function()
            require("conform").setup({
                formatters_by_ft = {
                    cpp = { "clang-format" },
                    c = { "clang-format" },
                    -- Другие языки используют свои LSP форматтеры автоматически
                },
                formatters = {
                    ["clang-format"] = {
                ...
                -- ВАЖНО: lsp_fallback = true значит для других языков используется LSP
                format_on_save = {
                    lsp_fallback = true,
                    timeout_ms = 500,
                },
    ...
    -- Линтер (nvim-lint)
    {
        ...
        config = function()
            require("lint").linters.cpplint = {
                cmd = "cpplint",
                args = { "--filter=-build/include_subdir", "--linelength=120" },
                stream = "stderr",
